...
type MotionDivAnimate = NonNullable<ComponentProps<typeof motion.div>["animate"]>;

const DEFAULT_CURSOR_COLOR = "#2f76b5";
const CURSOR_EASE = [0.625, 0.05, 0, 1] as const;

const customCursorVariants = cva("relative", {
  variants: {
    layout: {
      default: "",
...
function resolveCursorAppearance(isHovering: boolean, color: string): MotionDivAnimate {
  if (!isHovering) {
    return {
      width: 16,
      height: 16,
...
function CustomCursor({
  ...
  useEffect(() => {
    ...
    const handlePointerOver = (event: PointerEvent) => {
      ...
      setIsHovering(Boolean(target) && !isDisabled);
    };
...
function CustomCursorTarget({ children, className, size, ...props }: CustomCursorTargetProps) {
